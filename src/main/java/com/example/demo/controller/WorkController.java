...
/*
 * 2022/5/5 ①この初期化処理を実行すると、ページリンク処理で勤退開始日と勤退終了日をnullで取得してしまい、
 * 			SQL文の実行エラーとなる不具合が発生するため、正式な修正版が完成するまでは、
 * 			暫定版として初期化処理をコメントアウトします。
 *
 ...
 *
 * 		②また、勤退年月がそもそもの検索処理の複雑化の始まりであるため、
 * 		正式な修正版が完成するまでは、画面から勤退年月を非表示にする。
 * 		（※内部的には、テキストボックスが空白の状態で残す）
 * 		②の修正処理は、『templates』(フォルダ)のwork/list.html側での処理となります。
*/
	}
